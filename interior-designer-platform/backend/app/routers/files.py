...
async def upload_file(
    ...
    await ws_manager.broadcast({"type": "file", "project_id": project_id, "event": "created", "payload": {"id": db_file.id, "project_id": db_file.project_id, "filename": db_file.filename, "url": db_file.url}})
    return db_file


@router.post("/{project_id}", response_model=schemas.FileOut, status_code=201)
async def upload_file_by_path(
    ...
    """Upload a file for a project (path-style). Same as POST /files with Form project_id."""
    if not _can_access_project(db, project_id, current_user):
        raise HTTPException(status_code=403, detail="Not allowed to upload to this project")
    os.makedirs(UPLOAD_DIR, exist_ok=True)
    ext = os.path.splitext(file.filename or "file")[1] or ""
